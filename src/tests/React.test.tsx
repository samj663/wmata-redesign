...
describe("React Tests", () => {
  ...
  beforeAll(async () => {
    await fetch("https://dcmetrobackend.samueljohnson.dev/api");
  });
  ...
  test("Station Component", async () => {
    ...
    const setStation = (t: any) => {
      station = t;
    };
    const setMarkers = (t: any) => {
      markers = t;
    };
    const setZoom = (t: any) => {
      zoom = t;
  ...
  test("Next Bus Table StopID = 1000031", async () => {
    ...
    var busResponse = await fetch(
      "https://dcmetrobackend.samueljohnson.dev/api/nextBus?stopid=1000031",
    );
  ...
  test("Next Arrivals Table station = Metro Center", async () => {
    var dom: any;
    var busResponse = await fetch(
      "https://dcmetrobackend.samueljohnson.dev/api/nextarrival?station=Metro%20Center",
    );
    var rawBus = await busResponse.json();

    await act(async () => {
      dom = render(
        <NextArrivalsTable station="Metro Center" includeTransf="true" />,
      );
    });

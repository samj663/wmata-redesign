...
export async function read_rail_schedule() {
  ...
  console.log(
    date.toLocaleString("it-IT", { timeZone: "America/New_York" }).toString() +
      " -- Starting rail schedule update...",
  );
  await get_static_data(
    `https://api.wmata.com/gtfs/rail-gtfs-static.zip?api_key=${process.env.WMATA_KEY}`,
    "./static_rail",
  );
  ...
  if (
    ...
    console.warn(
      "WARNING: Checked GTFS rail schedule. Date change found. Updating bus database.",
    );

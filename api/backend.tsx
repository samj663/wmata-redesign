...
export async function bootstrap_get_data(){
    ...
    if(status ==="ERROR"){
        if(bootstrap_retry_counter.stations_fares_entrances >= MAX_RETRY){
            shutdown("Max number of fetches during startup exceeded. Shutting down...n/ Cause: bootstrap_get_data");
        }
...
export async function bootstrap_bus_stops(){
    ...
    if(status ==="ERROR"){
        if(bootstrap_retry_counter.bus_stops >= MAX_RETRY){
            shutdown("Max number of fetches during startup exceeded. Shutting down...n/ Cause: bootstrap_bus_stop");
        }
